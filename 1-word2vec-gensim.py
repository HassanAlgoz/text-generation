...
quadgram = Phrases(trigram[bigram[sentences]])
fivegram = Phrases(quadgram[trigram[bigram[sentences]]])
ticker.tock()
...
if os.path.exists(PATH.GENSIM_MODEL):
    ...
    model = Word2Vec(fivegram[quadgram[trigram[bigram[sentences]]]], size=args.EMBEDDING_HIDDEN_SIZE, window=args.EMBEDDING_WINDOW_SIZE, max_vocab_size=args.MAX_VOCAB_SIZE,  min_count=5, workers=os.cpu_count())
    ticker.tock()

ticker.tick("Gensim Training")
model.train(fivegram[quadgram[trigram[bigram[sentences]]]], total_words=num_words, epochs=args.EMBEDDING_EPOCHS)
ticker.tock()
